...
- **当前核心特性就绪度清单**：
  ...
  - [x] Rtodo 时间网格拖拽框选创建日程系统与全局手势管道贯通（宿主转发 `mouse_down` 拖拽与 `mouse_released` 释放手势、周视图与日视图支持按住鼠标直接纵向拖选时段、实时呈现发光液态玻璃选区与随动时间胶囊、松开鼠标秒级唤出新建弹窗并精准预填起止时间、无缝兼容单机操作与反向向上拖拽）；
  - [x] Rclock 24小时/12小时极简多维时钟图应用开发与底层亚像素扇区引擎（原生接入桌面抽屉与 Dock 栏、24H/12H 自由切换、昼夜环境光晕、Rtodo 标签色彩环形扇区映射、实时时间激光红线指针、中央液晶多功能控制核、右侧联动日程流与一键标记完成、完全直连 rtodo_data.bin 双向数据热同步）；
  - [x] 独立应用关闭内存深度回收与最小化流体云后台驻留系统（Red X 物理释放 win->inst 彻底回收内存，Yellow - 保持 win->inst 驻留并收入流体云后台，实现安卓级后台任务管理）；
  - [x] 流体云后台任务中心与多任务卡片管理器（常态呈现驻留应用微型图标徽标，展开时呈现流体云多任务中心：单卡片恢复、单任务关闭与一键清理所有后台任务）；
  - [x] 流体云横向经典 42px 胶囊布局回归与右侧从右往左后台任务排布（彻底摒弃纵向沉重面板，左侧保留红黄绿三色控制灯，右侧自右向左动态挂载任务胶囊，点击恢复/点击×回收内存/一键清理）；
  - [x] Rclock 界面彻底净化移除跳转按钮（时钟图顶栏去除「打开 Rtodo」按钮，专注纯粹沉浸式时间洞察与可视化）；
  - [x] 流体云无字化纯图符交互重塑（彻底剔除「系统流体云 · 暂无后台任务」、应用名中文标签与「清理」文字，任务胶囊紧凑化为 38x28px 纯图符微胶囊，右侧保留微型 × 与清理几何图符，实现极致纯粹无字流体交互）；
  - [x] 收回态流体云纯净 22x22 呼吸微核与后台程序桌面实时窗口预览（收回时彻底消除右侧缩略图与徽标，常态保持纯粹 22x22 呼吸晶球；展开流体云悬停后台任务胶囊时在桌面呈现完整实时窗口预览）；
  - [x] 物理级内存深度回收与操作系统工作集紧凑引擎（红灯 X 彻底关闭与流体云后台清理调用 `plugin->destroy` 释放堆对象，同时结合 `HeapCompact` 与 `SetProcessWorkingSetSize` 深度紧凑工作集，物理内存占用直降至 ~5MB，物理级回收真实有效）。
...
| **Milestone 26** | **Rtodo 时间网格拖拽框选创建日程系统与全局手势管道贯通 (Time Grid Drag-to-Select Event Creation & Global Gesture Pipeline)**：<br>1. **系统级手势事件管道打通 (`src/main.c`)**：扩展 `desktop_launcher_update`，当活动窗口处于激活态且未在拖拽窗口框架时，将鼠标按住拖动（`mouse_down[0] && !mouse_pressed[0]`）与鼠标释放（`mouse_released[0]`）事件无缝分发至活动窗口，让独立应用具备处理拖拽框选、长按与手势交互的能力。<br>2. **时间网格按住拖选与实时动态液态玻璃预览 (`src/app_calendar.c`)**：在周视图与日视图的时间轴空白网格上按住鼠标左键即可启动拖拽框选，光标所经之处实时呈现微透光高光蓝色液态玻璃选区卡片与顶边镜面高光；随动悬浮时间提示胶囊（Floating Time Badge）实时反馈正在框选的时间范围与总时长（如 `10:15 - 11:30 (1小时15分)` 或 `14:00 - 14:45 (45分钟)`）；支持反向向上拖拽，系统自动识别并将较早时刻作为开始时间、较晚时刻作为结束时间。<br>3. **松开鼠标秒级唤起创建弹窗与起止时间精准预填**：松开鼠标（`mouse_released[0]`）时瞬间唤起新建日程弹窗，自动将框选的起止时间精确预填入 `new_hour`, `new_min`, `new_end_hour`, `new_end_min`；完美兼容单击操作：当检测到无位移轻点时，自动赋予 15 或 30 分钟默认时长，兼顾极速单击新建与精准拖拽框选。 | `src/main.c`, `src/app_calendar.c`, `AI_CONTINUITY.md`, `walkthrough.md` |
| **Milestone 27** | **Rclock 24小时/12小时多维极简时钟图应用开发、底层亚像素圆弧扇面引擎与 Rtodo 双向数据热同步 (Rclock Radial Clock Chart Application, Subpixel Annular Sector Engine & Rtodo Real-time Bi-directional Synchronization)**：<br>1. **底层渲染引擎指令扩充 (`src/rife_core.h`, `src/rife_core.c`, `src/main.c`)**：新增 `CMD_CIRCLE`（SDF 亚像素抗锯齿连续圆）、`CMD_LINE`（亚像素抗锯齿胶囊线段/实时激光指针）、`CMD_ARC_SECTOR`（亚像素环形扇面渲染器，基于三角极坐标与双边缘平滑覆盖率，完美支持高光白边与 Alpha 半透通道），彻底根除圆形与圆弧阶梯锯齿。<br>2. **数据层解耦与双向零堆热同步 (`src/app_calendar.h`, `src/app_calendar.c`)**：将 `RtodoStorage` 结构与原子读写函数（`rtodo_load_storage`、`rtodo_save_storage`、`rtodo_get_storage_path`）正式公开抽象，统一挂载于 `%APPDATA%\RifeOS\rtodo_data.bin`；Rclock 与 Rtodo 完全共享底层数据，Rclock 内置文件时间戳监控，Rtodo 变动即刻毫秒级热刷新。<br>3. **Rclock 时钟图核心应用 (`src/app_clock.h`, `src/app_clock.c`)**：<br>   - 原生接入桌面抽屉与 Dock 栏（`g_clock_plugin_app`，暮光靛蓝渐变图标与“Rc”高光徽标；Rtodo 图标徽标同步升级为高辨识度“Rt”徽标，全局采用 GDI `DrawTextW` 绝对居中光栅化）；<br>   - 24 小时全日制与 12 小时制经典表盘一键切换；<br>   - 24H 模式下配备昼夜环境分界微光（06:00~18:00 白昼极光蓝，18:00~06:00 深邃子夜黑紫）；<br>   - 遍历并渲染当前日期的任务环形扇区（Annular Arc Sectors），完美映射 Rtodo 分类标签色彩；<br>   - 实时时间高光宝石红激光红线指针与顶端核晶圆点（动态指向当前小时+分+秒）；<br>   - 中央液晶多功能控制核（实时数字时钟、正在进行中任务卡片、下个日程倒计时、今日总工时统计）；<br>   - 右侧 330px 联动日程流（列表 Hover 联动表盘扇面高光，表盘 Hover 浮动详情卡片，圆形勾选框直接切换完成并写盘，支持点击圆盘空白处极速新建日程）。 | `src/rife_core.h`, `src/rife_core.c`, `src/main.c`, `src/app_calendar.h`, `src/app_calendar.c`, `src/app_clock.h`, `src/app_clock.c`, `src/app_manifest.h`, `src/app_manifest.c`, `CMakeLists.txt`, `AI_CONTINUITY.md`, `walkthrough.md` |
| **Milestone 28** | **Rclock 新建日程彻底解耦、应用间一键唤起与多维深度时间数据分析看板 (Rclock Creation Decoupling, Cross-App Activation & Multi-Dimensional Time Analytics Dashboard)**：<br>1. **新建日程彻底解耦归口 Rtodo (`src/app_clock.c`)**：完全移除 Rclock 中的新建模态框、打字输入流、空白表盘点击新建等逻辑；用户专注于在 Rtodo 中进行精密日程管理，Rclock 专注于纯粹的时钟可视化与数据分析；<br>2. **跨应用极速唤起联动 (`src/main.c`, `src/app_manifest.h`)**：新增系统级 API `rife_open_app_by_id(const char* app_id)`；在 Rclock 顶栏原新建按钮位置换装「打开 Rtodo」品牌蓝渐变胶囊按钮，点击即刻唤起/聚焦 Rtodo 窗口，实现双应用无缝协同；<br>3. **右侧 330px 面板双标签系统 (`src/app_clock.c`)**：<br>   - **Tab 0: 日程流**：顶部整合紧凑型概览条（`今日规划 X.Xh · 完成度 XX%`）与亚像素进度条；保持卡片列表与直接勾选完成态并回写持久化；空状态温馨提示前往 Rtodo 规划；<br>   - **Tab 1: 📊 深度数据分析看板**：<br>     - **卡片 1：完成效率与负荷**（超大完成率百分比大字、已完成/总项数、全天工时/空闲时间/利用率三联微指标）；<br>     - **卡片 2：分类工时占比矩阵**（全彩连续堆叠进度条 Stacked Color Bar、按标签显示圆点、标签名、日程数、累计时长与精准占比）；<br>     - **卡片 3：昼夜节律负荷分布**（凌晨 00~06、晨间 06~12、午后 12~18、晚间 18~24 四生理时段柱状对比与峰值标记）；<br>     - **卡片 4：智能时间画像与动态洞察**（动态分析用户当前负载、专注高峰时段并输出作息建议）。 | `src/app_clock.c`, `src/main.c`, `src/app_manifest.h`, `AI_CONTINUITY.md`, `walkthrough.md` |
| **Milestone 29** | **Rclock 顶栏彻底净化、独立应用内存深度回收与流体云安卓级后台多任务管理系统 (Rclock Purification, Granular Memory Reclamation & Fluid Cloud Android-style Multitasking Hub)**：<br>1. **Rclock 界面极致净化 (`src/app_clock.c`)**：彻底剔除顶栏原「打开 Rtodo」按钮及点击响应，右侧胶囊模式切换器（24H/12H）自然右靠齐（`client_w - 116.0f`，宽 100px），视觉回归极致克制纯粹；空状态与智能画像引导语更新为温馨指引；<br>2. **应用全生命周期分级内存回收架构 (`src/main.c`)**：<br>   - 在 `ActiveWindow` 中新增 `bool is_minimized;` 状态标识；<br>   - **真关闭（Red `X` 按钮）**：标记 `is_open = false; is_minimized = false;`，窗口收拢动画完成后物理执行 `win->plugin->destroy(win->inst); win->inst = NULL;`，彻底回收插件内存；<br>   - **真最小化（Yellow `-` 按钮）**：标记 `is_open = false; is_minimized = true;`，保留 `win->inst` 实例与运行时状态（包括滚轮位置、未提交输入、视图状态等），将其送入流体云后台驻留池；<br>   - 桌面抽屉与 Dock 栏唤起已驻留应用时，直接唤醒现有 `win->inst`，实现 0 内存开销、0 状态丢失的秒级前台恢复；唤起新应用时自动将原活动窗口转入后台最小化驻留；<br>3. **流体云多任务中心原型**：建立后台任务识别管线，支持多任务生命周期调度。 | `src/app_clock.c`, `src/main.c`, `AI_CONTINUITY.md`, `walkthrough.md` |
| **Milestone 30** | **流体云经典横向 42px 胶囊布局回归与右侧从右往左后台任务排布 (Fluid Cloud Classic 42px Horizontal Layout Restoration & Right-to-Left Task Docking)**：<br>1. **经典 42px 横向流体云布局全面回归 (`src/main.c`)**：彻底剔除纵向下拉大面积重型面板，展开高度严格锁死为原生的 `exp_h = 42.0f`，宽度随任务自适应（110px + tasks_w），保持桌面极致通透与灵动轻盈；<br>2. **左侧经典三色控制灯完整保留 (`src/main.c`)**：展开状态下左侧严格保留红、黄、绿三颗亚像素连续超椭圆圆钮（红灯退出系统、黄灯最小化桌面、绿灯全屏切换），交互逻辑 100% 忠实原生；<br>3. **右侧自右向左动态任务挂载引擎 (`从右往左加`, `src/main.c`)**：<br>   - 展开状态下从右侧边缘（`cloud_x + cloud_w - 10.0f`）起步，向左依次挂载每个后台驻留应用的精美横向玻璃任务胶囊（`96x28px`）；<br>   - 任务胶囊内嵌 20x20 渐变高清程序图标、中英文应用名称以及红色悬停微型 `×` 关闭按钮；<br>   - 点击胶囊主体即刻恢复对应窗口至前台（原前台窗口平滑降级驻留后台）；点击 `×` 即刻物理析构实例彻底释放内存；<br>   - 当后台任务数 `>= 2` 时，在最左侧任务胶囊之左自动呈现晶莹的「清理」按钮，一键释放全部后台实例并重置驻留池；<br>4. **静止态微岛自适应排布**：非展开状态下若有后台任务，胶囊保持 24px 极度精致微胶囊，左侧保留呼吸灯，右侧从右往左排布各个微型应用图标，一目了然；<br>5. **WM_NCHITTEST 视口物理对齐**：精准基于动态宽度与固定 42px 高度计算点击穿透区域，确保流体云点击区域与桌面无缝融合。 | `src/main.c`, `AI_CONTINUITY.md`, `walkthrough.md` |
| **Milestone 31** | **流体云彻底净化无字化、纯图符紧凑胶囊与极简几何后台管理 (Fluid Cloud Textless Purification, Pure Iconographic Capsule & Geometric Task Management)**：<br>1. **中英文冗余文字彻底剔除 (`src/main.c`)**：彻底移除展开流体云中间空闲提示语「系统流体云 · 暂无后台任务」/「Fluid Cloud · Ready」，空闲时中央保持纯净流动光感玻璃；彻底清除任务胶囊上的中英文应用名称标签（如「多维日程」、「极简时钟」）；彻底移除清理按钮上的「清理」/「Clear」字样；<br>2. **38x28px 纯图符微型任务胶囊 (`src/main.c`)**：后台任务胶囊宽度由 96px 大幅紧凑精炼至 **38px**，左侧内嵌 20x20 极简程序图标（高辨识度「Rt」、「Rc」或原生图符），右侧搭配微型亚像素 `×` 关闭角标（悬停微红高亮），点击图符即刻将应用恢复至前台，点击 `×` 即刻物理回收内存并激发生理光子吸收涟漪；<br>3. **纯几何符号一键清理按键 (`src/main.c`)**：当驻留后台应用数 `>= 2` 时，在最左侧浮现 26x28px 液态玻璃紧凑按键，内部采用 GDI 原生物理线条绘制纯净几何十字交叉图符（悬停变红高亮），0 字体依赖、0 乱码豆腐块风险，一键清空全部后台驻留并彻底释放内存；<br>4. **全套几何排布与命中判定像素级对齐 (`src/main.c`)**：同步校准 CPU 渲染 pass、GDI 绘制 pass、`WM_NCHITTEST` 穿透检测与鼠标点击分发逻辑，确保所有纯图符胶囊与几何按键在各种屏幕分辨率与缩放比例下 100% 灵敏响应。 | `src/main.c`, `AI_CONTINUITY.md`, `walkthrough.md` |
